...
function updateVisibility ( deltaTicks )
	...
	if pedSobelShader then 
		dxSetShaderValue( pedSobelShader, "sVisibility", fadeCurrent )
	end
	for _,shaderName in pairs(myShader) do
		if shaderName then
		dxSetShaderValue( shaderName, "sVisibility", fadeCurrent )
		end
	end
...
shineDirectionList = {
			...
			{  6, 40,	-0.914400,	0.377530,	-0.146093,	4,			0.0,		0 },			-- Sun fade in start
			{  6, 50,	-0.914400,	0.377530,	-0.146093,	4,			1.0,		0 },			-- Sun fade in end
...
function updateShineDirection ()
	...
	for idx,v in ipairs( shineDirectionList ) do
		...
		if fhoursNow <= fhoursTo then
			...
			local sunColor = {0,0,0}
			if fhoursNow > 6.2 and fhoursNow < 23.9 then
			sunColor[0], sunColor[1], sunColor[2], _ , _, _ = getSunColor()
			else sunColor[0] = 255 sunColor[1] = 255 sunColor[2] = 255 end
		
			-- Update shaders
			if pedSobelShader then 
				dxSetShaderValue( pedSobelShader, "sLightDir", x, y, z )
				dxSetShaderValue( pedSobelShader, "sSpecularPower", sharpness )
				dxSetShaderValue( pedSobelShader, "sSpecularBrightness", brightness )
				dxSetShaderValue( pedSobelShader, "sSunColor", sunColor[0]/255 , sunColor[1]/255 , sunColor[2]/255 ,sunBrightness)
				end
			for _,shaderName in pairs(myShader) do
			if shaderName then
				dxSetShaderValue( shaderName, "sLightDir", x, y, z )
				dxSetShaderValue( shaderName, "sSpecularPower", sharpness )
				dxSetShaderValue( shaderName, "sSpecularBrightness", brightness )
				dxSetShaderValue( shaderName, "sSunColor", sunColor[0]/255 , sunColor[1]/255 , sunColor[2]/255 ,sunBrightness)
				end
			end
