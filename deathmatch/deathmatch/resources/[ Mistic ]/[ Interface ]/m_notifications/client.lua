...
function noti:constructor()
	...
	self.fonts = {}
        self.fonts[1] = exports["m_gui"]:getFont("Montserrant_r", 10/zoom)
	self:code()
	self.nots = {}
end
...
function noti:render()
	for i,v in ipairs(self.nots) do
		...
		if getElementData(localPlayer, "auth:login") or getElementData(localPlayer, "auth:register") then
			...
			dxDrawRectangle(10+15/zoom - anim, sh - 274/zoom - add - 25, 380/zoom, 60/zoom, tocolor(30, 30, 30, alpha200))
			dxDrawText(v.text, 65+15/zoom - anim, sh - 272/zoom - add*2 - 50, 295/zoom + 65/zoom - anim, 60/zoom + sh - 272/zoom, tocolor(255, 255, 255, alpha), 1, self.fonts[1], "left", "center", true, true)
			dxDrawRectangle(10+15/zoom - anim, sh - 214/zoom - add - 25, 380/zoom, 2, tocolor(10, 10, 10, alpha))
			dxDrawRectangle(10+15/zoom - anim, sh - 214/zoom - add - 25, 380/zoom/100*bar, 2, tocolor(self.settings[v.type]["color"][1], self.settings[v.type]["color"][2], self.settings[v.type]["color"][3], alpha))
			dxDrawImage(25+15/zoom - anim, sh - 255/zoom - add - 25, 25/zoom, 25/zoom, string.format("assets/%s.png", v.type), 0, 0, 0, tocolor(self.settings[v.type]["color"][1], self.settings[v.type]["color"][2], self.settings[v.type]["color"][3], alpha))
		end
	end
end
