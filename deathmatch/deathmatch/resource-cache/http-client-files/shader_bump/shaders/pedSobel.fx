...
float4 sSunColor = float4(0,0,0,0);
bool isSobel = false;
...
VertexShaderOutput VertexShaderFunction(VertexShaderInput VS)
{
    ...
    PS.Tangent = mul(normalize(Tangent), gWorld);
    PS.Binormal = mul(normalize(Binormal), gWorld);
    PS.Normal = mul(normalize(VS.Normal), gWorld);
...
float4 PixelShaderFunctionSM3(VertexShaderOutput PS) : COLOR0
{
	...
	float4 normal = 1;
	if (isSobel)  { normal = float4(ComputeNormalsPS(PS.TexCoord.xy,saturate(1-color)),1); 
        normal.xy = normal.xy * 2.0 - 1.0;
        } else { normal = 1;}
        normal.w = normal.w * sEffIntens.y + 1.0;
    float3 normalMap = PS.Normal;
    if (isSobel) { normalMap += (normal.x * PS.Tangent + normal.y * PS.Binormal )* sqrt(1.0 - dot(normal.xy, normal.xy)); }
    float dotDir=0;	
    ...
    return PS.Diffuse * color + PS.Fresnel * color + color * float4(sunLight * sSunColor.a + diffuse * specular * normal.z * sEffIntens.x,1);
}
...
float4 PixelShaderFunctionSM2(VertexShaderOutput PS) : COLOR0
{
    ...
    return PS.Diffuse * color + PS.Fresnel * color + color * float4(sunLight * sSunColor.a + diffuse * specular * sEffIntens.x,1);
}
